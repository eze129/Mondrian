...
def split_categorical(partition, dim, pwidth, pmiddle):
    ...
    splitVal = ATT_TREES[dim][partition.middle[dim]]#获取当前分区在当前维度的泛化值
    # print(splitVal)
    ...
    """
    这里为gentree的定义中添加了is_member函数
        def is_member(self, record_value):
        # 如果节点有子节点,检查record_value是否属于子节点
        if self.child:
            for child in self.child:
                if child.is_member(record_value): 
                    return True
        # 如果节点没有子节点，检查记录值是否为该节点
        return self.value == record_value
    """
    for record in partition.member:
        record_value = record[dim] 
        for i, node in enumerate(sub_node):
            if node.is_member(record_value):#判断是否为record_value的父节点
                sub_groups[i].append(record)#record归于i对应node的组中
                break
    # print(len(sub_groups))
    for i, group in enumerate(sub_groups):
        if len(group) > 0:
            new_width = pwidth.copy()
            new_middle = pmiddle.copy()
            new_middle[dim] = sub_node[i].value
            new_width[dim] =  sub_node[i].leaf_num
            """
            sub_node[i].leaf_num与len(sub_node[i])均可，__len__(sub_node[i])为内置方式，可以自定义len求解方法
            """
            
            #print(sub_node[i].leaf_num)

            # 创建新的分区
            sub_partitions.append(Partition(group, new_width, new_middle))
